...
export const message = pgTable(
  "message",
  {
    ...
    metadata: jsonb("metadata").$type<CustomUIMessageData>().default({}),
  },
  (table) => [
...
export const starredModel = pgTable(
  "starred_model",
  {
    ...
    order: integer("order").default(0).notNull(),
    createdAt: timestamp("created_at").defaultNow().notNull(),
    supportsToolCalls: boolean("supports_tool_calls").default(false).notNull(),
    metadata: jsonb("metadata").$type<Model | null>().default(null),
  },
